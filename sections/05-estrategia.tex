...
\emph{Elija y justifique} los niveles aplicables a su proyecto: no todos los proyectos ejecutan
los cuatro. Un nivel que no se ejecuta debe declararse como no aplicable con su motivo, no
omitirse en silencio. Separe con claridad componente, integración, sistema y aceptación: no son
sinónimos ni etapas intercambiables.
\end{instruccion}

\begin{observacion}[Los cuatro niveles no son obligatorios]
\obsproblema{La redacción anterior enumeraba unidad, integración, sistema y aceptación de forma
que sugería que debían ejecutarse todos, sin pedir justificación ni permitir descartar alguno.}
\obscambio{El cuadro incorpora una columna «¿Se aplica?» y exige justificar tanto los niveles
elegidos como los descartados.}
\obsfuente{OBS §5.2}
\end{observacion}
...
procedimientos (TD6). Saltar de los elementos de prueba directamente a los casos hace perder
los eslabones intermedios, que son los que vuelven verificable la cobertura.
\end{notanormativa}

\begin{observacion}[Faltaban las condiciones y los elementos de cobertura]
\obsproblema{La plantilla anterior pasaba de los elementos de prueba a los casos sin registrar
las condiciones de prueba ni los elementos de cobertura, de modo que no había forma de comprobar
que los casos derivaran de la base de prueba y de los riesgos.}
\obscambio{Nuevo apartado con un cuadro de condiciones de prueba (TD2) y sus elementos de
cobertura (TD3), que alimenta la columna «condición» de la matriz del \ref{anx:trazabilidad}.}
\obsfuente{OBS §3 · PDF §1.4 (TD1–TD6)}
\end{observacion}
...
\ifmostrardiagramas
\input{diagrams/tecnicas-prueba}
...
\begin{notanormativa}[Los objetivos de cobertura no proceden de la norma]
\norma\ no fija un porcentaje mínimo de cobertura de sentencias ni de decisiones. Un objetivo
numérico es, a lo sumo, una decisión del proyecto o una política local, y debe declararse con su
justificación en el apartado~\ref{sec:cobertura}. Lo que caracteriza a cada técnica es el
\emph{elemento de cobertura} que produce, no un porcentaje\sidiag{: así se presentan en la
Figura~\ref{fig:tecnicas}}. Alcanzar una cobertura determinada tampoco demuestra la ausencia de
defectos.
\end{notanormativa}

\begin{observacion}[Cobertura mínima del diagrama de técnicas]
\obsproblema{El diagrama de técnicas de referencia presentaba «cobertura mínima: 100\,\%» para
sentencias y decisiones, atribuyendo implícitamente ese umbral a \norma.}
\obscambio{En el mapa de técnicas se retiraron los porcentajes y se sustituyeron por el
elemento de cobertura de cada técnica; el objetivo numérico se traslada al
apartado~\ref{sec:cobertura}, donde debe justificarse.}
\obsjustificacion{No hay base en los materiales examinados para atribuir ese mínimo universal a
la norma.}
\obsfuente{OBS §5.4 · DIAG}
\end{observacion}
...
distinto de los habituales, agréguelo al Cuadro~\ref{tab:tecnicas} con la misma justificación.
No estar entre las técnicas más frecuentes no lo excluye.
\end{instruccion}
...
\begin{notanormativa}[Una cobertura sin denominador no significa nada]
Un porcentaje de cobertura es ininteligible si no se declara sobre qué se mide. Conviene evitar
además un error frecuente: \textbf{la proporción de casos ejecutados es avance de ejecución, no
cobertura}. Ejecutar el 90\,\% de los casos no significa haber cubierto el 90\,\% de los
requisitos, de las particiones ni de las decisiones del código.
\end{notanormativa}

\begin{notanormativa}[Tres preguntas distintas: avance, cobertura y veredicto]
Al reportar una prueba se mezclan con frecuencia tres magnitudes que responden a preguntas
diferentes y que \segunvariante{esta plantilla mantiene}{este plan mantiene} separadas de
principio a fin:
...
\begin{tabular}{|L{2.6cm}|L{5.0cm}|L{5.4cm}|}
  \hline
  \filaenc \textbf{Magnitud} & \textbf{Pregunta que responde} & \textbf{Dónde se registra} \\ \hline
  \textbf{Avance de ejecución} & ¿Cuánto del trabajo planificado se ha realizado ya? & Registro de ejecución (\ref{anx:ejecuciones}) y seguimiento (\ref{sec:seguimiento}) \\ \hline
  \textbf{Cobertura} & ¿Qué elementos han quedado \emph{ejercitados}? & Este apartado y \ref{anx:trazabilidad} \\ \hline
  \textbf{Veredicto} & De lo ejercitado, ¿qué se comportó como se esperaba? & Registro de ejecución (\ref{anx:ejecuciones}) e incidentes (\ref{anx:incidentes}) \\ \hline
\end{tabular}
\end{center}
\medskip

De aquí se sigue una regla que conviene respetar al definir cualquier medida del cuadro
siguiente: \textbf{el numerador de una medida de cobertura nunca debe condicionarse al
veredicto}. Un requisito ejercitado por un caso que \emph{falló} está cubierto —la prueba lo
alcanzó y además encontró algo—; contarlo como no cubierto haría que la cobertura
\emph{bajara} al detectar defectos, que es justo lo contrario de lo que la medida pretende
indicar. Si necesita saber cuántos elementos se comportaron correctamente, ésa es una medida
de resultado y se reporta aparte.
\end{notanormativa}
...
\begin{tabularx}{\textwidth}{|C{1.5cm}|L{2.4cm}|Y|C{1.5cm}|C{2.0cm}|}
  ...
  \campo{COB-01} & \campo{Requisitos} & \campo{Requisitos con al menos un caso ejecutado, sea cual sea su veredicto ÷ requisitos del alcance} & \campo{100 \%} & \campo{Anexo G} \\ \hline
  ...
  & & & & \\ \hline
\end{tabularx}
\notatabla{Elementos posibles: requisitos · escenarios · particiones de equivalencia · valores límite · sentencias · decisiones · ramas · estados y transiciones · reglas de una tabla de decisión.\par
Compruebe que ningún numerador contenga las palabras «aprobado», «correcto» o «sin defectos»: eso convertiría la medida en un resultado de prueba. Un elemento ejercitado por un caso fallido \textbf{sí} está cubierto.}
\end{table}
...
ejercitar. No indica cuántos requisitos se comportaron correctamente —eso lo dicen los
veredictos del Cuadro~\ref{tab:resultados}—, ni que el software esté libre de defectos, ni que
los casos diseñados hayan sido adecuados.»
\end{instruccion}
...
\begin{observacion}[Cobertura sin denominador y contaminada por el veredicto]
\obsproblema{La plantilla anterior exigía porcentajes de cobertura (100/80/60/40) sin declarar
sobre qué elemento se calculaban, y trataba la proporción de casos ejecutados como si fuera
cobertura. En una revisión posterior se detectó además que la medida COB-01 propuesta se
definía sobre «requisitos con al menos un caso \emph{aprobado}», lo que mezclaba cobertura con
resultado de prueba.}
\obscambio{Cada medida debe declarar elemento, numerador, denominador, fórmula, objetivo, fuente
e interpretación. COB-01 se redefine sobre elementos \emph{ejercitados}, con independencia del
veredicto, y se añade la nota que separa avance, cobertura y veredicto.}
\obsjustificacion{Con el numerador condicionado al veredicto, la cobertura bajaba al encontrar
defectos: un requisito probado por un caso que falla contaba como no cubierto, ocultando que la
prueba sí se realizó.}
\obsfuente{OBS §4.4 · PROPIA}
\end{observacion}
...
\begin{notanormativa}[Para qué sirven los criterios de entrada]
Sin criterios de entrada no puede decidirse cuándo es legítimo empezar a ejecutar, y las
ejecuciones fallidas por causas evitables —versión equivocada, entorno no listo, datos
ausentes— se confunden con defectos del producto. La actividad \textbf{TE1} de \normap{2}
incluye explícitamente «verificar el cumplimiento de los criterios de entrada» antes de
ejecutar.
\end{notanormativa}

\begin{observacion}[No existían criterios de entrada]
\obsproblema{La plantilla anterior no preveía ningún criterio de entrada: definía cuándo
terminar de probar, pero no cuándo era legítimo empezar.}
\obscambio{Nuevo apartado con seis criterios mínimos (CE-01 a CE-06), cada uno con su evidencia
y su responsable de verificación, más el registro efectivo en el \ref{anx:entorno}.}
\obsfuente{OBS §3 y §6 · PDF §1.4 (TE1)}
\end{observacion}
...
\end{tabularx}
\notatabla{Estos seis criterios son un mínimo recomendado; amplíelos según el proyecto. Si un ciclo se inicia sin cumplir alguno, regístrelo como desviación (\ref{sec:desviaciones}).}
\end{table}
...
\begin{notanormativa}[Qué define y qué no define este apartado]
Conviene no mezclar tres cosas distintas: (i)~haber \emph{completado las actividades}
planificadas, (ii)~los \emph{resultados obtenidos}, y (iii)~la \emph{decisión de aceptación} del
producto. Este apartado define únicamente lo primero: cuándo se dan por terminadas las
actividades de prueba. Los resultados se reportan en el apartado~\ref{sec:resultados} y la
decisión de aceptación, en el apartado~\ref{sec:aceptacion}.
\end{notanormativa}

\begin{observacion}[Finalizar por cobertura alcanzada]
\obsproblema{La plantilla anterior resolvía la finalización por cobertura alcanzada,
argumentando que en el curso no se reparan los defectos encontrados, y la Parte~II derivaba de
esa cifra el «nivel de garantía de calidad logrado».}
\obscambio{Este apartado se limita a cerrar actividades; los criterios se clasifican en cuatro
tipos (avance, cobertura, resultados e incidentes) y la aceptación pasa a un apartado propio con
autoridad identificada.}
\obsjustificacion{Ejecutar todos los casos, aun cuando fallen, no demuestra que el producto sea
aceptable: la decisión debe considerar defectos abiertos, severidad, resultados y riesgo
residual.}
\obsfuente{OBS §4.6}
\end{observacion}
...
Combine criterios de los \textbf{cuatro} tipos, porque cada uno responde a una pregunta
distinta y ninguno sustituye a los demás: \emph{avance} (¿se hizo el trabajo previsto?),
\emph{cobertura} (¿se ejercitó lo que había que ejercitar?), \emph{resultados} (¿qué veredicto
se obtuvo?) e \emph{incidentes} (¿qué quedó sin resolver?). Un criterio de cobertura no informa
sobre el veredicto: si sólo exige cobertura, un conjunto de pruebas íntegramente fallido
cumpliría el criterio de salida.

Indique qué ocurre si un criterio no se alcanza: suspender, ampliar el esfuerzo o cerrar
documentando la desviación.
\end{instruccion}
...
\begin{tabularx}{\textwidth}{|C{1.5cm}|C{2.4cm}|Y|C{3.0cm}|}
  ...
  \campo{CS-01} & \campo{Cobertura} & \campo{COB-01 ≥ 100 \% en los objetos de criticidad Catastrófico (todos ejercitados, con independencia del veredicto)} & \campo{Ampliar / documentar desviación} \\ \hline
  \campo{CS-02} & \campo{Avance de ejecución} & \campo{Todos los procedimientos planificados ejecutados o formalmente cancelados} & \campo{Acción} \\ \hline
  \campo{CS-03} & \campo{Resultados} & \campo{Todo requisito Catastrófico con al menos una ejecución de veredicto \emph{aprobado} sobre la última versión probada} & \campo{Acción} \\ \hline
  \campo{CS-04} & \campo{Incidentes} & \campo{Ningún incidente de severidad crítica sin analizar} & \campo{Acción} \\ \hline
  & & & \\ \hline
\end{tabularx}
\end{table}

El Cuadro~\ref{tab:criterios-salida} separa los criterios de avance, de cobertura, de
resultados y de incidentes, de modo que el cierre no dependa de una sola cifra y, en
particular, para que ninguna medida de cobertura tenga que cargar con una afirmación sobre la
calidad del producto. Su evaluación efectiva se reporta en el
apartado~\ref{sec:evaluacion-salida}, y la decisión de aceptación —distinta de todo lo
anterior— en el apartado~\ref{sec:aceptacion}.
...
\begin{notanormativa}[Confirmación y regresión no son lo mismo]
\begin{itemize}
...
\end{itemize}
Llamar «regresión» a cualquier verificación de una corrección oculta que la segunda actividad
—comprobar que no se rompió nada más— puede no estarse realizando.\sidiag{ La
Figura~\ref{fig:procesos} representa ambas por separado.}
\end{notanormativa}

\begin{observacion}[«Verificar correcciones» rotulado como regresión]
\obsproblema{El diagrama de procesos de referencia asociaba la actividad «verificar
correcciones» a la prueba de regresión, y el apartado equivalente de la plantilla anterior
pedía «investigar cuál es la diferencia» en lugar de exigir que se planificaran ambas.}
\obscambio{Las dos actividades se definen y planifican por separado, con un cuadro comparativo
que obliga a fijar disparador, criterio de selección y responsable de cada una, y el diagrama
del flujo del proceso las dibuja como cajas distintas.}
\obsfuente{OBS §5.5 · DIAG}
\end{observacion}
...
Además de \emph{describir} el entorno, prevea cómo se \emph{comprueba} que está listo, quién lo
mantiene, cómo se controlan sus versiones y cómo se restaura a un estado conocido entre
ejecuciones. Estas tareas corresponden a las actividades \textbf{ES1 «Establecer el entorno»} y
\textbf{ES2 «Mantener el entorno»} de \normap{2}. El registro de cada preparación concreta se
lleva en el \ref{anx:entorno}.
\end{instruccion}

\begin{observacion}[El entorno se describía pero no se verificaba]
\obsproblema{La plantilla anterior pedía especificar los requisitos del entorno y de los datos,
pero no preveía comprobar que estuvieran listos antes de ejecutar, ni versionarlos, ni
restaurarlos entre ejecuciones.}
\obscambio{Se añade un cuadro de gestión del entorno (preparación, verificación, versiones,
mantenimiento, restauración y liberación), los datos ganan versión y tratamiento de
confidencialidad, y la verificación efectiva se registra en el \ref{anx:entorno}.}
...
Los datos también tienen versión: debe poder saberse qué conjunto de datos, y en qué versión,
se usó en cada ejecución; de lo contrario un resultado no puede reproducirse ni discutirse.
\end{instruccion}
...
\begin{notanormativa}[Por qué se versionan todos los activos]
Para que un resultado sea reproducible debe poder identificarse la versión de \emph{todos} los
activos que intervinieron en él, no sólo la del documento. La actividad \textbf{TP9} de
\normap{2} exige colocar el plan bajo gestión de configuración, y \textbf{TE3} exige registrar
versión y configuración en cada ejecución.
\end{notanormativa}

\begin{observacion}[Control de versiones limitado al documento]
\obsproblema{La plantilla anterior sólo preveía el historial de versiones del archivo Word. No
permitía saber qué versión del software, de los casos, de los guiones, de los datos o del
entorno se usó en cada ejecución.}
\obscambio{Nuevo apartado con un cuadro de ubicación, esquema de versión y responsable por tipo
de activo, que da sentido a las columnas «versión» y «configuración» del \ref{anx:ejecuciones}.}
\obsfuente{OBS §3 · PDF §1.3 (TP9), §1.4 (TE3)}
\end{observacion}
...
repositorio), basta con indicar dónde está y quién la mantiene.
\end{instruccion}
...
seis últimos son los que permiten reconstruir qué se probó, en qué condiciones y con qué
resultado.

\begin{observacion}[Entregables sin formato]
\obsproblema{La plantilla anterior fijaba cuatro entregables —plan, reporte de finalización,
casos y procedimientos— y no preveía ningún registro de lo ocurrido durante la ejecución.}
\obscambio{La lista pasa a nueve entregables, con ubicación y responsable, e incorpora los
registros de ejecución, incidentes, informes de estado, preparación de entorno y trazabilidad.}
\obsfuente{OBS §3 y §6}
\end{observacion}
