...
(emitir directivas y, si procede, replanificar) y reportar. Este proceso opera de forma
\emph{continua y concurrente} con la ejecución\sidiag{, como muestra la
Figura~\ref{fig:procesos}}: no es una etapa que ocurra entre dos entregas, y un listado de
fechas de entrega no lo sustituye.
\end{notanormativa}

\begin{observacion}[«Puntos de revisión» en lugar de seguimiento]
\obsproblema{La plantilla anterior resolvía este apartado con un listado de puntos de revisión
—las tres entregas del curso y sus fechas—. Eso cubre los hitos, pero no dice qué se mide, con
qué frecuencia, quién compara lo planificado con lo observado, ni qué se hace cuando aparece una
desviación.}
\obscambio{El apartado se convierte en una sección completa con cinco subapartados: enfoque de
monitoreo, hitos, registro de seguimiento por periodo, reglas de control y replanificación, y
plan de comunicación. Se añade el formato de informe de estado del \ref{anx:informes}.}
\obsjustificacion{Sin un registro periódico de avance previsto frente a real, las desviaciones
sólo se detectan en la entrega siguiente, cuando ya no admiten corrección.}
\obsfuente{OBS §3 y §5.6 · PDF §1.3 (TMC1–TMC4)}
\end{observacion}
...
\begin{tabularx}{\textwidth}{|L{3.6cm}|Y|C{2.4cm}|C{2.1cm}|}
  ...
  Cobertura alcanzada & \campo{Medidas COB-xx del Cuadro~\ref{tab:cobertura}: elementos ejercitados, con independencia del veredicto} & \campo{Semanal} & \campo{Rol} \\ \hline
...
El número de entregas es una regla del curso: los puntos de revisión deberían fijarse por
necesidad de decisión de los interesados, no por un número predefinido.\ifversionfinal\else\ Al
reutilizar la plantilla fuera del curso, sustitúyalo por los hitos que el proyecto requiera.\fi
...
\endlastfoot
  ...
  Cobertura alcanzada         & \campo{COB-xx = \% (ejercitado)} & \campo{} \\ \hline
...
El Cuadro~\ref{tab:control} cierra el bucle de control\sidiag{ representado en la
